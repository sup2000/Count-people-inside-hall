...
while True:
    ...
    if new_pts.ravel()[0] > 400 or new_pts.ravel()[2] > 400:
        if new_pts.ravel()[0] > 570 or new_pts.ravel()[2] > 570:
            new_pts = backup.copy()
            new_inp_img = backup_img.copy()
            ytest_pos += 10
            count = count+1
            #cv2.putText(outp, "gone at {}".format(datetime.datetime.now().strftime("%H:%M")), (10,ytest_pos),
                #cv2.FONT_HERSHEY_PLAIN, 0.5, (0,255,0))
            cv2.putText(outp, "No. of people in the hall : {}".format(count), (10, ytest_pos),
                        cv2.FONT_HERSHEY_PLAIN, 0.5, (0,255,0))
        ...
        if new_pts.ravel()[0] < 50 or new_pts.ravel()[2] < 50:
            ...
            #cv2.putText(outp, "came at {}".format(datetime.datetime.now().strftime("%H:%M")), (10,ytest_pos),
                ...
                cv2.putText(outp, "Room is empty!", (10, ytest_pos),
                            cv2.FONT_HERSHEY_PLAIN, 0.5, (0, 0, 255))
